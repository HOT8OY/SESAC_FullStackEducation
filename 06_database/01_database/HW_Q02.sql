...
FROM
	employee AS emp
INNER JOIN
	job AS jb ON emp.JOB_CODE = jb.JOB_CODE
WHERE
	jb.JOB_NAME = '대리'
    AND ENT_YN = 'N'
ORDER BY
...
SELECT
	dp.DEPT_TITLE AS '부서명',
    COUNT(JOB_CODE) AS '인원',
    SUM(SALARY) AS '급여합계',
    AVG(SALARY) AS '급여평균'
FROM
	employee AS emp
JOIN department AS dp ON emp.DEPT_CODE = dp.DEPT_ID
WHERE
	ENT_YN = 'N'
GROUP BY
	dp.DEPT_TITLE
...
FROM employee AS emp
	JOIN job AS jb ON jb.JOB_CODE = emp.JOB_CODE
    JOIN department AS dept ON dept.DEPT_ID = emp.DEPT_CODE
ORDER BY
	HIRE_DATE;
...
SELECT
	COUNT(*)
FROM employee AS mg
	INNER JOIN employee AS emp ON emp.EMP_ID = mg.MANAGER_ID;
...
SELECT
    ...
    dptmg.DEPT_TITLE
FROM employee AS emp
	LEFT JOIN employee AS mg ON emp.MANAGER_ID = mg.EMP_ID
    JOIN department dpt ON emp.DEPT_CODE = dpt.DEPT_ID
	JOIN department dptmg ON mg.DEPT_CODE = dptmg.DEPT_ID
ORDER BY
	emp.EMP_NAME;

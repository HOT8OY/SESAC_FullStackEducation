...
public class Application1 {
    public static void main(String[] args) {
        ...
        즉, 2차원 배열을 1차원 배열 여러 개를 하나로 묶어서 관리하는 배열이다.
        더 많은 차원의 배열을 사용할 수 있지만 일반적으로 2차원 배열 이상의 배열은 사용 빈도가 현저히 적다.

        [ 2차원 배열을 사용하는 방법]
        1. 배열의 주소를 보관할 레퍼런스 변수 선언 (stack 공간에)
        2. 여러 개의 1차원 배열의 주소를 관리할 배열 생성 (heap)
        3. 2에서 생성한 배열의 각 인덱스에서 관리하는 배열을 할당(heap)하여
           주소를 보관하는 배열 (= 2에서 생성한 배열)에 저장한다.
        4. 생성한 여러 개의 1차원 배열에 접근하여 사용                     */

        // # 1. 2차원 배열 선언 및 할당
        // 2차원 배열은 []을 두개 선언.
        // int[레퍼런스 배열 길이][각 배열의 길이]
        int[][] iarr = new int[3][5];   // 3개짜리 주소를 보관할 레퍼런스 배열을 만들고, 5개의 값이 들어갈 수 있는 배열을 할당.

        // # 1-1. 2차원 배열 선언과 동시에 할당 및 초기화(정변, 가변 둘 다 가능)
        // 각 층에 들어갈것을 {중괄호}안에 넣어준다.
